...
- G02.2 `PARTIAL`: machine-readable draft, bounded history diagnostic runtime verified (E30), and pure next-open/20-session research label arithmetic offline verified (E32). Existing historical validation is reused (E31), not restarted. Real exchange-calendar/price-policy binding, availability evidence, membership, export integration and contract freeze remain pending. No training authorization.
