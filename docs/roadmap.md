...
- G10.6 `PENDING`: spare-machine LLM/runtime inventory and approved cleanup, followed by dependency/health verification. Preflight checklist is 0/5 complete; it is tracked inside G10, not a new weighted product goal:
  ...
  - C2 `PARTIAL`: source dependencies inspected; typed preview/comparison now default to Qwen 1.5B only. Explicit legacy overrides remain supported. `GetSpareLlmDependencyReview.ps1` is offline-tested tooling; actual running service/job and pending experiment dependencies still need spare evidence and operator confirmation. Preserve all completed evidence. An engine (Ollama/llama.cpp) is not the same thing as an installed model.
