...
Latest scoped delivery E65: **mapping and corrected spare replay verified**, 600 mapped rows/150 dates, 1.336s on PS7.6.6, zero training-eligible rows. [Contract and acceptance](numerical-research-mapping.md). No repeat run/rebuild or new Java/data/model call requested. Next is the grouped source/availability and learner/evaluation contract stage; E62's restricted research scope is unchanged. This is preparation within G02/G03, not completion of either full goal. Source policy, production availability/evaluation criteria and fitting approval remain open; dashboard percentages below are unchanged.
