...
| E32 | Pure NumericalResearchLabelCalculator, seven new Java tests; GetExistingDataValidationEvidence and offline PS test | OFFLINE VERIFIED / NOT WIRED TO EXPORT | Full Java suite 268 tests passed. Nine PS5.1 assertions: known-file selection, metric/hash preservation, malformed/oversized reports, missing evidence, redaction, one-file output and no HTTP. Calculator tests next-open/20th-close arithmetic, fractional costs/losses, no shifted missing bars, invalid/non-executable prices, calendar/policy guards and outside-horizon isolation. No actual market labels, dataset export, model fitting or new source validation performed. Saved evidence collector runtime pending |
