...
public class UpstoxMarketDataService {
    ...
    public UpstoxImportResult importNseEquityInstruments() {
        ...
        if (accepted.isEmpty()) {
            markSourceFailure();
            return new UpstoxImportResult("INVALID_DATA", fetch.data().size(), 0, fetch.data().size(),
    ...
    public UpstoxImportResult importHistoricalCandles(UpstoxHistoricalRequest request) {
        ...
        if (instrumentId == null) {
            return new UpstoxImportResult("INSTRUMENT_NOT_IMPORTED", fetch.data().size(), 0,
                    fetch.data().size(), "Import the NSE instrument master before candle ingestion.");
        }

        List<UpstoxCandle> accepted = fetch.data().stream().filter(qualityService::validCandle).toList();
        for (UpstoxCandle candle : accepted) {
            jdbcTemplate.update("""
                    INSERT INTO market_candle
                        (instrument_id, source_id, interval_code, opened_at, source_published_at,
        ...
        return new UpstoxImportResult("SUCCESS", fetch.data().size(), accepted.size(),
                fetch.data().size() - accepted.size(),
                "Historical candles passed OHLC, timestamp, and volume validation before persistence.");
    }
