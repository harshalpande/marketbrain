...
The next data-first layer is a read-only point-in-time feature preview. It selects one canonical daily candle per date, prefers reviewed official NSE BhavCopy remediation over Upstox when both exist, removes governed feature-exclusion windows, and computes deterministic trend, momentum, volatility, range, and volume features only from observations at or before an explicit `asOf` date. A universe preview streams the same calculation across all 500 matched instruments, classifies current, stale, insufficient-history, and no-data results, and binds the complete ordered output to an immutable SHA-256 manifest. No feature row, signal, order, or other database state is written. Stale or incomplete data remains non-actionable, and the system still cannot create a real broker order.
