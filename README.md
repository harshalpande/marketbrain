...
The next data-first layer computes point-in-time technical features. It selects one canonical daily candle per date, prefers reviewed official NSE BhavCopy remediation over Upstox when both exist, removes governed feature-exclusion windows, and computes deterministic trend, momentum, volatility, range, and volume features only from observations at or before an explicit `asOf` date. A universe preview streams the same calculation across all 500 matched instruments, classifies current, stale, insufficient-history, and no-data results, and binds the complete ordered output to an immutable SHA-256 manifest. An explicitly reviewed manifest can then be persisted atomically as 500 immutable classifications: complete vectors for eligible instruments and feature-free withheld records for ineligible instruments. Persistence is idempotent and cannot generate signals or orders. A read-only daily handoff preview additionally requires a completed incremental job, clean database quality, all 500 target-date Upstox candles, and a point-in-time-safe feature analysis before declaring automatic persistence ready. Stale, incomplete, or unresolved data remains non-actionable, and the system still cannot create a real broker order.
