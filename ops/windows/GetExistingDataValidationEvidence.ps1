...
$report=[pscustomobject]@{
    version='EXISTING_DATA_VALIDATION_EVIDENCE_V1';status='RUNNING';createdAtUtc=[DateTime]::UtcNow.ToString('o');updatedAtUtc=$null
...
try {
    ...
    $matching=@($listed | Where-Object {$_.Name -match '^(final-provider-quality-|expansion-batch-[0-9]+-.*(quality|analysis|investigation|remediation)|daily-enrichment-(database-quality|provider-quality|quality-checkpoints)-|remaining-data-analysis-).*\.json$'} | Sort-Object LastWriteTimeUtc -Descending)
    ...
    foreach($file in $matching){
        ...
        $entry=[pscustomobject]@{name=$file.Name;bytes=$file.Length;lastWriteTimeUtc=$file.LastWriteTimeUtc.ToString('o');sha256=$null;status='PENDING';metrics=$null;errorType=$null}
        $report.files.Add($entry)
        if(($file.Attributes -band [IO.FileAttributes]::ReparsePoint) -or $file.Length -gt 10MB -or ($bytesRead+$file.Length) -gt 50MB){
            ...
            try {
                ...
                try {
                    if($stream.Length -ne $file.Length -or $stream.Length -gt 10MB){throw 'File changed before read.'}
    ...
    $report.summary=[pscustomobject]@{selectedFiles=$matching.Count;inspectedFiles=$report.files.Count;bytesRead=$bytesRead;capturedSummaries=@($report.files.ToArray() | Where-Object status -eq 'SAVED_METRICS_CAPTURED_NOT_REVALIDATED').Count}
