...
$path=Join-Path (Resolve-Path -LiteralPath $OutputDirectory).Path ('numerical-evaluation-smoke-{0}-{1}.json' -f (Get-Date -Format 'yyyyMMdd-HHmmss'),$id.Substring(0,12))
...
try {
    ...
    if($Runtime -eq 'Java'){
        ...
        $arguments=@('-Xmx256m','--source','21',$source,'--synthetic-smoke')
    } else {
        ...
        $arguments=@('run','--rm','--pull','never','--name',$containerName,'--network','none','--read-only',
            ...
            $image,'-Xmx256m','--source','21','/input/NumericalEvaluationEngineering.java','--synthetic-smoke')
        $dockerStarted=$true
    }
    Update-EvaluationProgress 40 "Running fixed synthetic suite via $Runtime; no fitting or external data."
