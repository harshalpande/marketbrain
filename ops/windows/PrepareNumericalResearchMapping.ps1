...
$report=[pscustomobject]@{version='NUMERICAL_RESEARCH_MAPPING_COLLECTION_V1';status='RUNNING';createdAtUtc=[DateTime]::UtcNow.ToString('o');updatedAtUtc=$null
    ...
    trainingAuthorized=$false;failureStage=$null;errorType=$null;scriptSha256=(Get-FileHash -LiteralPath $PSCommandPath).Hash
...
try{
    ...
    if($SavedMappingResultPath){
        ...
        $report.reusedResultSha256=$reused.sha256;$report.result=$reused.data
    }else{
    ...
    $stage='REVIEW';Save-MappingProgress 80 'Response checkpointed; checking every vector, date group, blocker and safety counter.'
